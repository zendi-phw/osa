...
/**
 * The main mod_osa move_down file.
 *
 * @package     mod_osa
...
// Set up page.
$PAGE->set_url($url);
$PAGE->set_title(get_string('pagetitlemovedown', 'mod_osa') . ' ' . format_string($osa->name));
...
$nextcategoryentrycategory = $nextsortentry->category;

if (empty($nextsortentryid)) {
    echo "no next entry available";
    // Return to view.php.
...
$DB->set_field('osa_qtype_collection', 'sort', $nextsortentrysort, ['id' => $currentsortentryid, 'fk_cmid' => $cmid]);
// Update current category entry id with next sort id. $currentsortentryid stays the same as it is the same record id.
$DB->set_field('osa_qtype_collection', 'category', $nextcategoryentrycategory, ['id' => $currentsortentryid, 'fk_cmid' => $cmid]);

// Update smaller sort entry id with former current sort id.
$DB->set_field('osa_qtype_collection', 'sort', $currentsortentrysort, ['id' => $nextsortentryid, 'fk_cmid' => $cmid]);
// Update next category entry id with former current category id. $nextsortentryid stays the same as it is the same record id.
$DB->set_field('osa_qtype_collection', 'category', $currentcategoryentrycategory, ['id' => $nextsortentryid, 'fk_cmid' => $cmid]);
